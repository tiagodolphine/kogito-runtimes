...
public class DMNRestResourceTemplate {
    ...
    public $outputType$ dmn(@RequestBody(required = false) $inputType$ variables,
        ...
        return $extractContextMethod$(result, response);
    }
    ...
    private Object extractContextIfSucceded(DMNResult result, HttpServletResponse httpResponse){
        if (!result.hasErrors()) {
            try {
                enrichResponseHeaders(result, httpResponse);
    ...
    private OutputSet extractStronglyTypedContextIfSucceded(DMNResult result, HttpServletResponse httpResponse) {
        if (!result.hasErrors()) {
            enrichResponseHeaders(result, httpResponse);
    ...
    private Object extractSingletonDSIfSucceded(DMNResult result, HttpServletResponse httpResponse) {
        if (!result.hasErrors()) {
            try {
                enrichResponseHeaders(result, httpResponse);
